...
func init() {
	android.RegisterModuleType("baikalos_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = baikalosExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = baikalosExpandVariables(ctx, dep_file)
	...
	cmd := baikalosExpandVariables(ctx, String(g.properties.Cmd))
