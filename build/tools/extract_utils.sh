...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$BAIKALOS_ROOT"/vendor/baikalos/build/tools/smali/baksmali.jar
        export SMALIJAR="$BAIKALOS_ROOT"/vendor/baikalos/build/tools/smali/smali.jar
    fi
...
function extract() {
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        DUMPDIR="$BAIKALOS_ROOT"/system_dump
